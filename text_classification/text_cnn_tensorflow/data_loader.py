...
def prepare_dataset(dataset=None, x_text=None, y=None):
    ...
    if dataset is not None:
        ...
        print("Total data length : ", len(dataset))  # todo 随机采样作为测试机，这里还可以优化（采样方法）
    ...
    for fragment in line.strip().lower().split():
        for token in re.split(_WORD_SPLIT, fragment):
            if not token:
                continue
            if normalize_digits:
                token = re.sub(_DIGIT_RE, '#', token)  # todo 将数字都替换成“#”符号
            words.append(token)
    return words
    ...
    def count_vocab(fname):
        with open(fname, 'rb') as f:
            for line in tqdm(f.readlines()):
                ...
                token_list = basic_tokenizer(line)
...
def make_train_and_test_set(shuffle=True):
    ...
    assert len(train_X) == len(train_y)  # todo 检查最后处理的数据，非常有必要
...
def load_data(X_fname, y_fname):
    ...
    X_data, y_data = [], []
    for X_line, y_line in tqdm(zip(X_input_data.readlines(), y_input_data.readlines()),desc="处理训练数据"):
        ...
        if len(X_ids) <= Config.data.max_seq_length:  # todo 长截短补
            X_data.append(_pad_input(X_ids, Config.data.max_seq_length))
        # todo 这里可以使用tf.one_hot处理，非常方便
    ...
    def get_inputs():
        ...
        def train_inputs():  # todo 数据的输入
            with tf.name_scope(scope):
                ...
                # Build dataset iterator    # todo tf.data高级api数据处理
...
def make_batch2(data, buffer_size=10000, batch_size=64, scope="train"):
    class IteratorInitializerHook(tf.train.SessionRunHook):
        """Hook to initialise data iterator after Session is created."""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default="kaggle_movie_review",
                        choices=["check_tiny", "kaggle_movie_review", "rt-polarity"],
